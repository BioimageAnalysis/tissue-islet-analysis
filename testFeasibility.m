...
maskIsletFull = false(reader.height, reader.width);
...
for ii = 1:prod(numTiles)
    ...
    maskIslets = (aa - spotColorLab(2)).^2 + (bb - spotColorLab(3)).^2 <= 20^2;
    maskIslets = imopen(maskIslets, strel('disk', 3));

    % maskTissue = (aa - tissueColorLab(2)).^2 + (bb - tissueColorLab(3)).^2 <= 20^2;
    % maskTissue = imopen(maskTissue, strel('disk', 3));
    maskTissue = Igreen < 180 & Igreen > 5;
